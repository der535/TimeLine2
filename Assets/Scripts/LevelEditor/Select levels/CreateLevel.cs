...
namespace TimeLine
{
    public class CreateLevel : MonoBehaviour
    {
        ...
        private LevelBaseInfo _levelInfo;
        ...
        private void TempFolder()
        {
            ...
            _tempFolderPath = myuuid.ToString();
            Directory.CreateDirectory($"{Application.persistentDataPath}/Levels/{_tempFolderPath}");
        }

        public void SelectMusic()
        {
            _audioSelectAudio.OpenFilePanel((value) =>
            {
                if (value.Count == 0) return;
            
                var musicPath = value[0];
            
                _songName = Path.GetFileName(musicPath);
            
                _textOnButtonLoadSong.text = _songName;
                File.Copy(musicPath,
                    $"{Application.persistentDataPath}/Levels/{_tempFolderPath}/{Path.GetFileName(musicPath)}",
                    overwrite: true);
        ...
        public void Create()
        {
            Directory.Move($"{Application.persistentDataPath}/Levels/{_tempFolderPath}",
        ...
        public void Cancel()
        {
            string folderPath = $"{Application.persistentDataPath}/Levels/{_tempFolderPath}";
