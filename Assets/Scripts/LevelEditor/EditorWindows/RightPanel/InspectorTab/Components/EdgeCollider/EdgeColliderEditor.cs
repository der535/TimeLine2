...
namespace TimeLine
{
    public class EdgeColliderEditor : MonoBehaviour
    {
        ...
        private EdgeColliderSelectionCube _edgeColliderSelectionCube;

        [Inject]
        void Construct(GridScene gridScene, GameEventBus eventBus, EdgeColliderSelectionCube edgeColliderSelectionCube)
        {
            _gridScene = gridScene;
            _gameEventBus = eventBus;
            _edgeColliderSelectionCube = edgeColliderSelectionCube;
        } 

        public void TurnEdit()
        {
            _isEditing = !_isEditing;
            _gameEventBus.Raise(new TurnEditColliderEvent(_isEditing));
        }
        ...
        void Update()
        {
            ...
            _edgeColliderSelectionCube.SetActive(isNearEdge);
            ...
            if (isNearEdge)
            {
                ...
                if (distToP1 <= distanceToCorner)_edgeColliderSelectionCube.SetPosition(worldP1); 
                else if (distToP2 <= distanceToCorner) _edgeColliderSelectionCube.SetPosition(worldP2); 
                else _edgeColliderSelectionCube.SetPosition(closestPointOnEdge);
            }
            ...
            if (!draggedPointIndex.HasValue && UnityEngine.Input.GetMouseButtonDown(0))
            {
                ...
                if (isNearEdge)
                {
                    Vector2 worldP1 = transform.TransformPoint(edgeCollider2D.points[pointOne]);
                    Vector2 worldP2 = transform.TransformPoint(edgeCollider2D.points[pointTwo]);
                    bool nearP1 = Vector2.Distance(_edgeColliderSelectionCube.GetPosition(), worldP1) <= distanceToCorner;
                    bool nearP2 = Vector2.Distance(_edgeColliderSelectionCube.GetPosition(), worldP2) <= distanceToCorner;
