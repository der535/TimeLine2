...
namespace TimeLine
{
    public class EdgeColliderEditorOLD : MonoBehaviour
    {
        EdgeCollider2D edgeCollider2D;
        LineRenderer lineRenderer;
        private GameObject edgeColliderObject;
        ...
        private float distanceToEdgeDynamic;
        private float distanceToCornerDynamic;
        private float intersectingSegmentWidthDynamic;
        ...
        List<GameObject> intersectingSegmentObjects = new();
        List<LineRenderer> radiusVisualizers = new();
        List<LineRenderer> edgeConnectionVisualizers = new();
        ...
        [Inject]
        void Construct(GridScene gridScene, GameEventBus eventBus,
            C_EdgeColliderSelectionCube cEdgeColliderSelectionCube,
            SceneToRawImageConverter sceneToRawImageConverter, C_EditColliderState cEditColliderState,
        ...
        private void Start()
        {
            _gameEventBus.SubscribeTo((ref EditorSceneCameraUpdateViewEvent data) => { CalculateDynamicFields(); });
        }

        internal void CalculateDynamicFields()
        {
            distanceToEdgeDynamic = distanceToEdgeBase * _cameraReferences.editSceneCamera.orthographicSize;
            distanceToCornerDynamic = distanceToCornerBase * _cameraReferences.editSceneCamera.orthographicSize;
            intersectingSegmentWidthDynamic =
                intersectingSegmentWidthBase * _cameraReferences.editSceneCamera.orthographicSize;

            if(lineRenderer == null) return;
            lineRenderer.startWidth = intersectingSegmentWidthDynamic;
            lineRenderer.endWidth = intersectingSegmentWidthDynamic;
            foreach (var VARIABLE in radiusVisualizers)
            {
                VARIABLE.startWidth = intersectingSegmentWidthDynamic;
                VARIABLE.endWidth = intersectingSegmentWidthDynamic;
            }
        ...
        internal void Init(ListVector2Parameter parameter, EdgeCollider2DComponent edgeCollider2DComponent, GameObject _edgeColliderObject)
        {
            edgeCollider2D = _edgeColliderObject.GetComponent<EdgeCollider2D>();
            lineRenderer = _edgeColliderObject.GetComponent<LineRenderer>();
            edgeColliderObject = _edgeColliderObject;
        ...
        public void UpdateOutline()
        {
            ...
            {
                lineRenderer.SetPosition(i, edgeColliderObject.transform.TransformPoint(points[i]));
            }
        }
        ...
        void UpdatePositions(int index, Vector2 worldPosition)
        {
            ...
            Vector2 localPosition = edgeColliderObject.transform.InverseTransformPoint(worldPosition);
            Vector2[] points = edgeCollider2D.points;
        ...
        {
            worldPosition = _gridScene.PositionFloatSnapToGrid(worldPosition, quaternion.identity);
            Vector2 localPoint = edgeColliderObject.transform.InverseTransformPoint(worldPosition);
        ...
        (int, int, float, Vector2) ClosestSegmentIndices(Vector2[] polyline, Vector2 point)
        {
            ...
            {
                Vector2 a = edgeColliderObject.transform.TransformPoint(polyline[i]);
                Vector2 b = edgeColliderObject.transform.TransformPoint(polyline[i + 1]);
        ...
        void Update()
        {
            if (lineRenderer == null || lineRenderer.enabled == false || _cEditState.GetState() == false) return;
            ...
            for (int i = 0; i < points.Length; i++)
            {
                float distSqr = Vector2.SqrMagnitude((Vector2)edgeColliderObject.transform.TransformPoint(points[i]) - mouseWorldPos);
            ...
            var (segA, segB, minEdgeDistSqr, closestPointOnEdge) = ClosestSegmentIndices(points, mouseWorldPos);

            // Параметры для принятия решения
            bool nearVertex = minVertexDistSqr <= distanceToCornerDynamic * distanceToCornerDynamic;
            bool nearEdge = minEdgeDistSqr <= distanceToEdgeDynamic * distanceToEdgeDynamic;

            // 3. Логика подсветки куба (приоритет вершине, если она близко)
            if (nearVertex)
            {
                _cEdgeColliderSelectionCube.SetActive(true);
                _cEdgeColliderSelectionCube.SetPosition(edgeColliderObject.transform.TransformPoint(points[closestVertexIndex]));
            }
            else if (nearEdge)
            {
                _cEdgeColliderSelectionCube.SetActive(true);
            ...
            if (!draggedPointIndex.HasValue && UnityEngine.Input.GetMouseButtonDown(0))
            {
                ...
                if (nearVertex)
                {
                    if (isCtrlPressed) RemovePoint(closestVertexIndex);
        ...
        void UpdateIntersectingSegments()
        {
            ...
            for (int i = 0; i < edgeCollider2D.points.Length; i++)
                worldPoints[i] = edgeColliderObject.transform.TransformPoint(edgeCollider2D.points[i]);
        ...
        GameObject GetSegmentObject()
        {
            ...
            newObj.transform.SetParent(edgeColliderObject.transform);
        ...
        void UpdateEdgeRadiusVisualization()
        {
            ...
            while (radiusVisualizers.Count < pointCount)
            {
                ...
                obj.transform.SetParent(edgeColliderObject.transform);
