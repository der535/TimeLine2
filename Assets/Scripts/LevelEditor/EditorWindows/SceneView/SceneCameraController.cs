...
namespace TimeLine
{
    public class SceneCameraController : MonoBehaviour
    {
        ...
        {  
            return RectTransformUtility.RectangleContainsScreenPoint(rawImage,  
        ...
        private void HandleZoom()
        {
            ...
            if (scrollDelta != 0)
            {
                Vector3? worldPointBefore = sceneToRawImageConverter.ScreenPointToWorldScene(UnityEngine.Input.mousePosition, 0);
                
                if (worldPointBefore.HasValue)
                {
                    // Экспоненциальный зум
                    float zoomFactor = Mathf.Pow(1.2f, -scrollDelta * zoomSensitivity);
                    sceneEditorCamera.orthographicSize = Mathf.Clamp(sceneEditorCamera.orthographicSize * zoomFactor, minSize, maxSize);

                    Vector3? worldPointAfter = sceneToRawImageConverter.ScreenPointToWorldScene(UnityEngine.Input.mousePosition, 0);
        ...
        private void HandlePan()
        {
            ...
            if (_isDragging)
            {
                Vector3? currentMouseWorld = sceneToRawImageConverter.ScreenPointToWorldScene(UnityEngine.Input.mousePosition, 0);
