    ...
    public class CopyComponentController
    {
        ...
        public void Copy(BaseParameterComponent component)
        {
            ...
            foreach (var track in _keyframeTrackStorage.GetTracks())
            {
                ...
                {
                    _copyTrack.Add((track.Track.Copy((GameObject)null), track.TreeNode.Path));
                }
            }
        }

        public void PasteNewComponent(GameObject targetToPaste)
        {
            // 1. Проверка на наличие данных в буфере
            if (_copyComponent == null || _copyParemetersData == null) return;

            var component =  (BaseParameterComponent) ComponentRules.AddComponentSafely(_copyComponent.Name, targetToPaste, _container);
