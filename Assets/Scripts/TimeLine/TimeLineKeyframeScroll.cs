...
namespace TimeLine
{
    public class TimeLineKeyframeScroll : MonoBehaviour
    {
        ...
        {
            Vector2 mousePosition = new Vector2(UnityEngine.Input.mousePosition.x, UnityEngine.Input.mousePosition.y) - _mainObjects.CanvasRectTransform.sizeDelta;
            
            // print(targetObject.sizeDelta);
            // print(mousePosition);
            
            if(Math.Abs(mousePosition.x) > targetObject.sizeDelta.x || Math.Abs(mousePosition.y) > targetObject.sizeDelta.y) return;
