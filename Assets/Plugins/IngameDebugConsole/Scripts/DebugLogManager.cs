...
#pragma warning restore 0414
        ...
        [SerializeField] internal TMP_FontAsset logItemFontOverride;

        [SerializeField] private TextMeshProUGUI commandSuggestionPrefab;
        ...
        [SerializeField] private TextMeshProUGUI infoEntryCountText;
        [SerializeField] private TextMeshProUGUI warningEntryCountText;
        [SerializeField] private TextMeshProUGUI errorEntryCountText;
...
#pragma warning restore 0649
        ...
        /// </summary>
        [System.NonSerialized] internal char[] textBuffer = new char[4096];
        ...
        private void Awake()
        {
            // 1. ПРОВЕРКА SINGLETON (ОДИНОЧКИ)
            // Делаем это в первую очередь, чтобы не выполнять лишний код для дубликатов
            if (Instance != null && Instance != this)
            {
                // Используем DestroyImmediate, чтобы объект гарантированно исчез 
                // до того, как Unity попытается зарегистрировать его ID
            ...
            if (Instance == null)
            {
                ...
                // Если включен режим синглтона, делаем объект вечным
                if (singleton)
                {
                    // Важно: DontDestroyOnLoad работает корректно только для корневых объектов
            ...
            // 2. ИНИЦИАЛИЗАЦИЯ КОЛЛЕКЦИЙ И ПЕРЕМЕННЫХ
            ...
            // 4. ПОДГОТОВКА БУФЕРОВ ДАННЫХ
            collapsedLogEntries = new DynamicCircularBuffer<DebugLogEntry>(128);
            collapsedLogEntriesMap =
                new Dictionary<DebugLogEntry, DebugLogEntry>(128, new DebugLogEntryContentEqualityComparer());
            ...
            recycledListView.Initialize(this, logEntriesToShow, timestampsOfLogEntriesToShow,
                logItemPrefab.Transform.sizeDelta.y);

            // 5. ВАЛИДАЦИЯ РАЗМЕРОВ И ИНТЕРФЕЙСА
    ...
    logItemsScrollRect.scrollSensitivity *= 0.25f;
        ...
        private void LateUpdate()
        {
            ...
            if (newInfoEntryCount > 0 || newWarningEntryCount > 0 || newErrorEntryCount > 0)
            {
                ...
                if (!isLogWindowVisible)
                {
                    ...
                    {
                        if ((newInfoEntryCount > 0 &&
                             (popupVisibilityLogFilter & DebugLogFilter.Info) == DebugLogFilter.Info) ||
                            (newWarningEntryCount > 0 && (popupVisibilityLogFilter & DebugLogFilter.Warning) ==
                                DebugLogFilter.Warning) ||
                            (newErrorEntryCount > 0 && (popupVisibilityLogFilter & DebugLogFilter.Error) ==
                ...
                if (isLogWindowVisible || timestampsOfLogEntriesToShow != null)
                {
                    ...
                    if (logEntryIndexInEntriesToShow >= 0)
                    {
                     ...
                     (logFilter == DebugLogFilter.All ||
                      ...
                      (logType != LogType.Log && logType != LogType.Warning &&
        ...
        private void RemoveOldestLogs(int numberOfLogsToRemove)
        {
            ...
            DebugLogEntry logEntryToSelectAndFocus =
                (indexOfLogEntryToSelectAndFocus >= 0 && indexOfLogEntryToSelectAndFocus < logEntriesToShow.Count)
            ...
            {
                collapsedLogEntries.RemoveAll(shouldRemoveCollapsedLogEntryPredicate,
                    updateLogEntryCollapsedIndexAction, collapsedLogEntriesTimestamps);

                if (isCollapseOn)
                    removedLogEntriesToShowCount = logEntriesToShow.RemoveAll(shouldRemoveLogEntryToShowPredicate, null,
        ...
        private void FilterLogs()
        {
            ...
            {
                DynamicCircularBuffer<DebugLogEntry> targetLogEntries =
                    isCollapseOn ? collapsedLogEntries : uncollapsedLogEntries;
        ...
        public string GetAllLogs(int maxLogCount, float maxElapsedTime)
        {
            ...
            if (uncollapsedLogEntriesTimestamps != null)
            {
                ...
                while (startIndex < uncollapsedLogEntries.Count &&
                       currentElapsedSeconds - uncollapsedLogEntriesTimestamps[startIndex].elapsedSeconds >
                       maxElapsedTime)
                    startIndex++;
            }
        ...
        public void GetAllLogs(out DynamicCircularBuffer<DebugLogEntry> logEntries,
